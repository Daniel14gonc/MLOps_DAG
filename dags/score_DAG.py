...
with DAG(
    ...
    def predict(data):
        print(data)
        try:
            response = requests.post('http://fastapi-backend:8001/predict/score_bulk', json=data)
    ...
    def bulk_update(data):
        try:
            response = requests.put('http://django-backend:8000/api/users/bulk-update-effort/', json=data)
            response.raise_for_status()
        except requests.RequestException as e:
            raise ValueError(f"Error al llamar a la API: {e}")
